...
Begin VB.UserControl IHScroll 
   ...
   End
End
Attribute VB_Name = "IHScroll"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = False
Attribute VB_Exposed = False
Option Explicit
Private Const nW = 18   '滚动条向左向右按钮的宽度
Private Const nMinGlideWidth = 10 '滑块的最小宽度
...
Private Declare Function BitBlt Lib "gdi32" (ByVal hDestDC As Long, ByVal X As Long, ByVal Y As Long, ByVal nWidth As Long, ByVal nWeight As Long, ByVal hSrcDC As Long, ByVal xSrc As Long, ByVal ySrc As Long, ByVal dwRop As Long) As Long
Private Declare Function DeleteDC Lib "gdi32" (ByVal hdc As Long) As Long
Private Declare Function SetStretchBltMode Lib "gdi32" (ByVal hdc As Long, ByVal nStretchMode As Long) As Long
Private Declare Function StretchBlt Lib "gdi32" (ByVal hdc As Long, ByVal X As Long, ByVal Y As Long, ByVal nWidth As Long, ByVal nWeight As Long, ByVal hSrcDC As Long, ByVal xSrc As Long, ByVal ySrc As Long, ByVal nSrcWidth As Long, ByVal nSrcHeight As Long, ByVal dwRop As Long) As Long
...
Private m_GlideWidth As Long    '滑块高度
...
Private m_GlideLeft As Long
Private m_srcHScroll As Long
...
Public Sub zSubclass_Proc(ByVal bBefore As Boolean, ByRef bHandled As Boolean, ByRef lReturn As Long, ByRef lhwnd As Long, ByRef uMsg As Long, ByRef wParam As Long, ByRef lParam As Long)
    ...
    Select Case uMsg
        ...
        Case WM_PAINT
            ...
            TmpDC = pCreateDC(ScaleWidth - 2 * nW, ScaleHeight)
            SetRect rc, 0, 0, ScaleWidth - 2 * nW, ScaleHeight
            pFillRect TmpDC, rc, &H1F1F1F
            SetStretchBltMode TmpDC, STRETCH_HALFTONE
            SetStretchBltMode uPS.hdc, STRETCH_HALFTONE
            
            Call pDrawGlideAndBkg(TmpDC)
            Call BitBlt(uPS.hdc, nW, 0, ScaleWidth - 2 * nW, ScaleHeight, TmpDC, 0, 0, vbSrcCopy)
        ...
        Case WM_LBUTTONDOWN, WM_LBUTTONDBLCLK
            ...
            dwX = LOWORD(lParam)
            m_OldX = dwX
            m_Index = DownTest(dwX)
            Call OnLbuttonDown(dwX)
...
Private Sub pDrawGlideAndBkg(ByVal mDC As Long)
    ...
    'Dim tmpGlideLeft As Long

    '贴背景图
    pStretchBlt mDC, 1, 0, ScaleWidth - nW * 2 - 2, ScaleHeight, m_srcHScroll, 32 * 6, 0, 32, 32, 1
            
    '贴滑块
    If m_GlideLeft < nW + 3 Then m_GlideLeft = nW + 3
    If m_GlideLeft > ScaleWidth - nW - 3 - GlideWidth Then
        m_GlideLeft = ScaleWidth - nW - 3 - GlideWidth
    End If
    
    'If m_GlideLeft > ScaleWidth - nW - GlideWidth - 6 Then
    '    tmpGlideLeft = ScaleWidth - nW - GlideWidth - 6
    'Else
    '    tmpGlideLeft = m_GlideLeft
    'End If
    
    If m_IsGlideDown Then
        '贴滑块按下的图
        pStretchBlt mDC, m_GlideLeft - nW, 2, GlideWidth, ScaleHeight - 4, m_srcHScroll, 32 * 5, 0, 32, 32, 1
    Else
        ...
        End If
        '贴正常时的图
        pStretchBlt mDC, m_GlideLeft - nW, 2, GlideWidth, ScaleHeight - 4, m_srcHScroll, 32 * 4, 0, 32, 32, 1
    End If
End Sub
...
Private Sub tmrIsLbuttonDown_Timer()
    ...
    If m_IsGlideDown Then
        If (PT.X - m_OldX) Then
            Dim tmpLeft As Long
            tmpLeft = m_GlideLeft
            m_GlideLeft = m_GlideLeft + (PT.X - m_OldX)
            If m_GlideLeft <= ScaleWidth - GlideWidth - nW And m_GlideLeft >= nW Then
                m_OldX = PT.X
            Else
                If m_GlideLeft > ScaleWidth - GlideWidth - nW Then
                    m_GlideLeft = ScaleWidth - GlideWidth - nW
                Else
                    m_GlideLeft = nW
                End If
                m_OldX = m_rel + m_GlideLeft
            End If
            Dim nValue As Long
            nValue = pGetValueByGlideLeft(m_GlideLeft)
            If m_OldValue <> nValue Then
                m_Value = nValue
                RaiseEvent Change
            End If
            If tmpLeft <> m_GlideLeft Then
    ...
    Select Case m_Index
        ...
        Case 2
            m_Value = m_Value + m_SmallChange
        Case 0
            If PT.X <= pGetGlideLeft(m_Value) Then
                m_Value = m_Value - m_LargeChange
                If PT.X > pGetGlideLeft(m_Value) Then IsStop = True
            ElseIf PT.X >= pGetGlideLeft(m_Value) + GlideWidth Then
                m_Value = m_Value + m_LargeChange
                If PT.X < pGetGlideLeft(m_Value) + GlideWidth Then IsStop = True
            End If
    End Select
    ...
    If m_OldValue <> m_Value Then
        m_GlideLeft = pGetGlideLeft(m_Value)
        InvalidateRect hwnd, 0, 0
        RaiseEvent Change
    End If
...
Private Function DownTest(ByVal dwX As Long) As Long
    If dwX >= 0 And dwX <= nW Then
        DownTest = 1
    ElseIf dwX >= ScaleWidth - nW And dwX <= ScaleWidth Then
        DownTest = 2
    ElseIf dwX > m_GlideLeft And dwX < m_GlideLeft + GlideWidth Then
        DownTest = 3
    Else
        DownTest = 0
    End If
...
Private Sub OnLbuttonDown(ByVal dwX As Long)
    ...
    Select Case m_Index
        ...
        Case 3
            m_IsGlideDown = True
            m_rel = dwX - pGetGlideLeft(Value)
            Exit Sub
        Case 0
            If dwX <= pGetGlideLeft(Value) Then
                m_Value = m_Value - m_LargeChange
            ElseIf dwX >= pGetGlideLeft(Value) + GlideWidth Then
                m_Value = m_Value + m_LargeChange
            End If
    End Select
    ...
    m_GlideLeft = pGetGlideLeft(Value)
    If m_OldValue <> m_Value Then
        InvalidateRect hwnd, 0, 0
        RaiseEvent Change
    End If
...
Private Function MoveTest(ByVal dwX As Long) As Long
    MoveTest = -1
    If dwX >= 0 And dwX <= nW Then
        MoveTest = 1
    ElseIf dwX >= ScaleWidth - nW And dwX <= ScaleWidth Then
...
Private Sub pDrawTopButton(ByVal hdc As Long)
    If m_IsLbuttonDown Then
        '贴按下时的图
        If m_Index = 1 Then
            pStretchBlt hdc, 0, 0, nW + 1, ScaleHeight, m_srcHScroll, 32, 0, 32, 32, 2
        End If
    Else
        ...
        Else
            '贴正常时的图
            pStretchBlt hdc, 0, 0, nW + 1, ScaleHeight, m_srcHScroll, 0, 0, 32, 32, 2
        End If
    End If
End Sub
...
Private Sub pDrawBottomButton(ByVal hdc As Long)
    If m_IsLbuttonDown Then
        '贴按下时的图
        If m_Index = 2 Then
            pStretchBlt hdc, ScaleWidth, 0, nW + 1, ScaleHeight, m_srcHScroll, 32 * 2, 0, 32, 32, 2
        End If
    Else
        If m_HoverIndex = 2 Then
            '贴鼠标在上方的图
            pStretchBlt hdc, ScaleWidth - nW, 0, nW + 1, ScaleHeight, m_srcHScroll, 32 * 2, 0, 32, 32, 2
        Else
            '贴正常时的图
            pStretchBlt hdc, ScaleWidth - nW, 0, nW + 1, ScaleHeight, m_srcHScroll, 32 * 2, 0, 32, 32, 2
        End If
...
Private Sub pDrawGlideArea(ByVal hdc As Long)
    '贴背景图
    pStretchBlt hdc, nW, 0, ScaleWidth - nW * 2, ScaleHeight, m_srcHScroll, 32 * 6, 0, 32, 32, 1
End Sub

'画滑块
Private Sub pDrawGlide(ByVal hdc As Long)

    If m_GlideLeft < nW Then m_GlideLeft = nW + 3
    If m_GlideLeft > ScaleWidth - nW - GlideWidth Then
        m_GlideLeft = ScaleWidth - nW - GlideWidth
    End If
    Debug.Print GlideWidth
    If m_IsGlideDown Then
        '贴滑块按下的图
         Debug.Print m_GlideLeft
        pStretchBlt hdc, m_GlideLeft, 0, GlideWidth, ScaleHeight, m_srcHScroll, 32 * 4, 0, 32, 32, 1
    Else
        If m_HoverIndex = 3 Then
            '贴鼠标在上方的图
            pStretchBlt hdc, m_GlideLeft, 0, GlideWidth, ScaleHeight, m_srcHScroll, 32 * 4, 0, 32, 32, 1
...
'得到滑块的Left值(比较抽象,用数学归纳法找出规律)
Public Function pGetGlideLeft(ByVal CurValue As Long) As Long
    
    If CurValue = MinV Then
        pGetGlideLeft = nW + 3
        Exit Function
    ElseIf CurValue = MaxV Then
        pGetGlideLeft = ScaleWidth - GlideWidth - nW - 3
        Exit Function
    End If
    ...
    If m_IsMoreThan Then
        If m_Mod Then
            ...
            If CurValue > MinV And CurValue <= TMP Then
                pGetGlideLeft = (CurValue - MinV) * (m_Count + 1) + nW + 3
            Else
                pGetGlideLeft = pGetGlideLeft(TMP) + (CurValue - TMP) * m_Count
            End If
        Else
            pGetGlideLeft = (CurValue - MinV) * m_Count + nW + 3
        End If
    Else
        If m_Mod Then
...
'根据滑块的Left值得到对应的Value值(近似)
Private Function pGetValueByGlideLeft(ByVal nGlideLeft As Long)
    
    Dim TMP As Long

    If nGlideLeft <= nW + 3 Then
        pGetValueByGlideLeft = MinV
        Exit Function
    ElseIf nGlideLeft >= ScaleWidth - GlideWidth - nW - 3 Then
        pGetValueByGlideLeft = MaxV
        Exit Function
    End If

    If m_IsMoreThan Then
        If m_Mod Then
            TMP = m_Mod * (m_Count + 1) + nW + 3
            If nGlideLeft > nW + 3 And nGlideLeft <= TMP Then
                pGetValueByGlideLeft = MinV + (nGlideLeft - nW - 3) \ (m_Count + 1)
            Else
                pGetValueByGlideLeft = MinV + m_Mod + (nGlideLeft - TMP) \ m_Count
            End If
        Else
            pGetValueByGlideLeft = (nGlideLeft - nW - 3) \ m_Count + MinV
        End If
    Else
        If m_Mod Then
            TMP = m_Mod * (m_Count + 1)
            If nGlideLeft > nW + 3 And nGlideLeft <= nW + 3 + m_Mod Then
                pGetValueByGlideLeft = MinV + (m_Count + 1) * (nGlideLeft - nW - 3)
            Else
                pGetValueByGlideLeft = MinV + TMP + m_Count * (nGlideLeft - nW - 3 - m_Mod)
            End If
        Else
            pGetValueByGlideLeft = (nGlideLeft - nW - 3) * m_Count + MinV
        End If
...
'得到m_GlideWidth,m_Mod和m_Count的值,还有m_GlideLeft的值
Private Sub pGetGlideInformation()

    Dim MaxCnt As Long
    Dim ActMaxCnt As Long
    MaxCnt = ScaleWidth - nW * 2 - nMinGlideWidth - 6
    
    If m_GlideWidth < nMinGlideWidth Then m_GlideWidth = nMinGlideWidth
    If m_GlideWidth >= MaxCnt Then
        'MsgBox "滑块高度值超过最大值,请适当调高滚动条的高度!", vbOKOnly, "提示"
        Exit Sub
    End If
    
    ActMaxCnt = ScaleWidth - nW * 2 - m_GlideWidth - 6
...
Private Sub UserControl_Initialize()
    ...
    m_HoverIndex = -1
    m_GlideLeft = nW
    m_srcHScroll = pCreateDCByHandle(LoadResPicture(102, vbResBitmap).handle)
    m_GlideWidth = nMinGlideWidth
End Sub
...
Public Property Get GlideWidth() As Long
    GlideWidth = m_GlideWidth
End Property

Public Property Let GlideWidth(ByVal va As Long)
    m_GlideWidth = va
End Property
